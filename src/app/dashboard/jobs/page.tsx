...
function PartyCell({ value }: { value: string | null }) {
  ...
  return (
    <td className="px-4 py-3.5 max-w-[220px]">
      ...
      {address && <p className="text-[10.5px] text-gray-400 dark:text-navy-500 truncate mt-0.5">{address}</p>}
    </td>
  );
}
    ...
    <motion.div variants={pageTransition} initial="hidden" animate="visible" className="space-y-4">
      <motion.div variants={staggerItem} className="flex flex-wrap items-center gap-3">
        <div>
          ...
          <p className="text-[11px] text-gray-400 dark:text-navy-500 mt-0.5">{data?.total ?? 0} total jobs</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <input
            ...
            className="text-[12px] border border-gray-200 dark:border-navy-700 rounded-xl px-3 py-2 bg-white dark:bg-navy-900 text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-navy-500 w-64 focus:outline-none focus:border-emerald-300 dark:focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-900/40"
          />
        </div>
      </motion.div>

      <motion.div variants={staggerItem} className="flex items-center bg-gray-100 dark:bg-navy-800 rounded-xl p-0.5 gap-0.5 w-fit">
        {STATUS_TABS.map(tab => (
          <button
            key={tab.key}
            ...
            className={`px-3 py-1.5 text-[11px] font-bold rounded-lg transition-all ${
              status === tab.key
                ? 'bg-white dark:bg-navy-700 text-emerald-700 dark:text-emerald-400 shadow-sm'
                : 'text-gray-400 dark:text-navy-400 hover:text-gray-600 dark:hover:text-navy-200'
            }`}
          >
            {tab.label}
          </button>
      ...
      {selected.size > 0 && (
        <motion.div
          ...
          className="flex items-center gap-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/60 rounded-xl px-4 py-2.5"
        >
          <span className="text-[12px] font-bold text-emerald-700 dark:text-emerald-400">{selected.size} job{selected.size !== 1 ? 's' : ''} selected</span>
          <button
            onClick={handleManifest}
            disabled={manifesting}
            className="ml-auto text-[12px] font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 px-4 py-1.5 rounded-lg transition-colors"
          >
            {manifesting ? 'Creating manifest…' : 'Create Manifest'}
          </button>
          <button
            onClick={() => setSelected(new Set())}
            className="text-[12px] font-semibold text-gray-500 dark:text-navy-400 hover:text-gray-700 dark:hover:text-navy-200"
          >
            Clear
          </button>
        </motion.div>
        ...
        <motion.div variants={staggerItem} className="bg-white dark:bg-navy-900 rounded-2xl border border-gray-100 dark:border-navy-800 shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100 dark:border-navy-800">
                ...
                {['HAWB', 'Shipper', 'Consignee', 'Weight (kg)', 'Status', 'Received'].map(h => (
                  <th key={h} className={`px-4 py-3 text-[10px] font-bold text-gray-400 dark:text-navy-500 uppercase tracking-wider whitespace-nowrap ${h === 'Weight (kg)' ? 'text-right' : ''}`}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-navy-800/70">
              {jobs.map(job => (
                <tr key={job.id} className="hover:bg-gray-50/70 dark:hover:bg-navy-800/50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 whitespace-nowrap">
                    <Link href={`/dashboard/jobs/${job.id}`} className="flex items-center gap-1.5 font-mono font-bold text-emerald-600 dark:text-emerald-400 text-[12px] hover:underline w-fit">
                      <FileText size={13} className="shrink-0 opacity-50" />
                      {job.hawb_number}
                    </Link>
                    {job.packages.length > 1 && (
                      <span className="mt-1 inline-flex items-center gap-1 text-[9px] font-bold text-gray-500 dark:text-navy-400 bg-gray-100 dark:bg-navy-800 px-1.5 py-0.5 rounded-full">
                  ...
                  <td className="px-4 py-3.5 text-[12px] text-gray-700 dark:text-navy-300 whitespace-nowrap text-right tabular-nums">{job.weight_kg ?? '—'}</td>
                  <td className="px-4 py-3.5 whitespace-nowrap">
                  ...
                  <td className="px-4 py-3.5 text-[11px] text-gray-500 dark:text-navy-500 whitespace-nowrap">
          ...
          {data && data.total_pages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-navy-800">
              <span className="text-[11px] text-gray-400 dark:text-navy-500">Page {data.page} of {data.total_pages}</span>
              <div className="flex gap-2">
                <button
                  onClick={() => setPage(p => Math.max(1, p - 1))}
                  disabled={page <= 1}
                  className="text-[11px] font-bold text-gray-500 dark:text-navy-400 bg-gray-50 dark:bg-navy-800 hover:bg-gray-100 dark:hover:bg-navy-700 disabled:opacity-40 px-3 py-1.5 rounded-lg transition-colors"
                >
                  Previous
                </button>
                <button
