  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="visible" className="space-y-4">
      <motion.div variants={staggerItem}>
        <h1 className="text-base font-black text-gray-900 leading-tight">Manifests</h1>
        <p className="text-[11px] text-gray-400 mt-0.5">{manifests.length} manifests</p>
      </motion.div>
              ...
              {manifests.map(m => (
                <tr key={m.id} className="hover:bg-gray-50/70 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap">
                    <Link href={`/dashboard/manifests/${m.id}`} className="font-mono font-bold text-amber-600 text-[12px] hover:underline">
                      {m.reference_number}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-gray-700">{m.job_count}</td>
                  <td className="px-4 py-3 text-[12px] text-gray-700">{m.total_weight_kg}</td>
