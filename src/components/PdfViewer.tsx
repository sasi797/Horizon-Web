...
export default function PdfViewer({
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100 rounded-2xl border border-gray-100 overflow-hidden">
      <div
      ...
      >
        <Document
          ...
          loading={<div className="text-xs text-gray-400 py-10">Loading PDF…</div>}
      ...
      {numPages > 1 && (
        <div className="flex items-center justify-center gap-3 py-2 border-t border-gray-200 bg-white shrink-0">
          <button
            onClick={() => setPageNumber(p => Math.max(1, p - 1))}
            disabled={pageNumber <= 1}
            className="text-[11px] font-bold text-gray-500 disabled:opacity-40 px-2 py-1"
          >
            ← Prev
          </button>
          <span className="text-[11px] text-gray-500 font-medium">{pagerLabel}</span>
          <button
